...
class Tractor:
    ...
    def find_last(self, row):
        col = 0
        while self.load_point(col, row) == 0:
            col += 1
        first = col
        while self.load_point(col, row) == 1:
            col += 1
        return col-1

    def is_square(self, col, row, width):
        return self.load_point(col, row) == 1 and self.load_point(col + width, row) == 1 and self.load_point(col, row + width) == 1 and self.load_point(col + width, row + width) == 1



if __name__ == '__main__':
    tractor = Tractor()
    tractor.load_region(50, 50)
    print(f"part 1: {tractor.affected}")

    tractor = Tractor()
    # found 6751081
    row = 1081
    col = tractor.find_last(row) - 99
    square = tractor.is_square(col, row, 99)
    print(f"row={col} col={row} fits={square}")
    print(f"part 2: {col * 10000 + row}")
